...
## Redesign Update
- Pivoted from kafka for the time being, using postgres to set up persistent storage
- Revamped fastapi server to handle valuation tasks in parallel and perform bulk db inserts. This is in tune with the end goal of
scaling to hundreds or thousands of stocks eventually..

### Summary of Design Considerations for Valuation and DB Insertion
The new server (main_clean.py) implements a producer-consumer pattern optimized for I/O-bound workloads. The valuation computation is decoupled into two stages: producers use a ThreadPoolExecutor to fetch stock data from the yfinance API concurrently (since yfinance is a blocking library), and each producer appends results to a thread-safe queue.Queue. A single async consumer task reads from this queue, batches results based on size and time thresholds, and performs efficient bulk inserts to PostgreSQL using asyncpg's executemany method. This hybrid approach leverages threads where necessary for blocking I/O (yfinance calls release the GIL, allowing true concurrency) while keeping database operations in the async domain for natural integration with asyncpg and FastAPI's event loop. The consumer task is lifecycle-bound to the computation method — it starts when processing begins and gracefully shuts down after all producers complete and the queue is drained, avoiding the overhead of perpetual background polling. The number of producer threads is configured based on available CPU cores scaled for I/O workloads, though in containerized environments this is explicitly set via environment variables to avoid issues with os.cpu_count() returning host CPU counts rather than container limits.

## Machine Learning
To create a robust investment engine, I'm trying to incorporate simple sentiment analysis models and later LLMS that will eventually supplement the valuations for each stock. The idea is to get a holistic picture of a stock. I decided to implement the model myself, using an LSTM encoder + feedforward neural network to predict sentiment for a headline. Playing around in Colab, my model was not much better than a simple model without an LSTM. Regardless, I will keep a couple models around (including a refined model from Hugging Face when the time comes). The tentative idea is to scrape data from some source (Reddit as of now) and set up a Flink job to process the data, send data to a microservice responsible for inference, and then push to kafka. This is all up in the air, and Flink may be overkill here. For the microservice, planning to use torch.load on the model I trained and saved in Colab. Will be interesting to see how much longer load takes for a smaller model vs a parameter heavy LSTM model. Will also be interesting to see latency for inference. I believe it is thread safe in some contexts, but haven't decided how exactly to set up the inference microservice and make it fast.

### Model Training
Playing around with the model accuracy, I started with an LSTM + feed forward neural net that yielded ~60 accuracy. The first improvement step was making the LSTM bidirectional, and this improved accuracy to 68%. The next step is to potentially use pretrained embeddings like Glove to improve it just to see how the accuracy improves...[update] this improved accuracy to 75% (which is good enough for now to use in the pipeline). [TODO] add in diagrams for models etc.
